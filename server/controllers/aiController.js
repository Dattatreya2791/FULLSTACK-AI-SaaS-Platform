...
export const generateArticle = async (req,res)=>{
    try {
        ...
        if (!prompt || typeof prompt !== "string" || !length || isNaN(length)) {
            return res.json({ success: false, message: "Invalid prompt or length." });
        }

        if(plan !== 'elite' && free_usage >= 10){
...
export const generateImage = async (req,res)=>{
    try {
        ...
        if(plan !== 'elite'){
            return res.json({success:false, message:"This feature is only available for Elite subscriptions"})
        }
...
export const removeImageBackground = async (req,res)=>{
    try {
        ...
        if(plan !== 'elite'){
            return res.json({success:false, message:"This feature is available for Elite subscriptions"})
        }
