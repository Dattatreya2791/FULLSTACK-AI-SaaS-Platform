...
const BlogTitles = () => {
    ...
    const blogCategories = ['General', 'Technology', 'Health', 'Lifestyle', 'Travel', 'Education', 'Bussiness', 'Finance', 'Food'];
    ...
    const onSubmitHandler = async (e) => {
      ...
      try {
        setLoading(true)
        const prompt = `Generate a blog title for the 
        keyword ${input} in the categoty ${selectedCategory}`
  ...
  return (
    ...
    flex-wrap gap-4 text-slate-700'>
      ...
      border border-gray-200'>
        ...
        rounded-md border border-gray-200' placeholder='The future of AI is...' required/>

        <p className='mt-5 text-sm font-medium'>Category</p>

        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-19/11'>
        ...
        <br/>
        <button disabled={loading} className='w-full flex items-center justify-center gap-2 bg-gradient-to-r 
        from-[#C341F6] to-[#8E37EB] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
          {loading ? <span className='w-4 h-4 my-1 rounded-full border-2
            border-t-transparent animate-spin'></span> : <Hash className='w-5 h-5'/>}
          Generate Title
        </button>
      </form>
